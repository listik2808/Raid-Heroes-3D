...
namespace Scripts.Army.TypesSoldiers
{
    public class Warrior : Soldier
    {
        ...
        public void SpecAttack(float meleeDamage, float duartionStan, Soldier soldier)
        {
            soldier.TakeDamage(meleeDamage);
        }
        ...
        public void SetAbilityValues(DataLevelSkill valueWarrior)
        {
            SetMaxSpecAttack();
            SpecialSkillLevelData.SetStepSkill((int)valueWarrior.CurrentStepSpecialAttack);
            SurvivabilityLevelData.SetStepSkill((int)valueWarrior.CurrentStepSurvivability);
            MeleeDamageLevelData.SetStepSkill((int)valueWarrior.CurrentStepMelee);
            SoldiersStatsLevel.SetSpecAttac((int)valueWarrior.CurrenSpecialAttackLevel);
            SoldiersStatsLevel.SetHealthLevel((int)valueWarrior.CurrenSurvivabilityLevel);
            SoldiersStatsLevel.SetMeleeLevel((int)valueWarrior.CurrenMeleeLevel);
            ...
            //_currentSpeed = DataSoldier.BaseSpeedValue + (SpeedSkillLevelData.ValueUpSpeed * valueWarrior.GetCurrentMobilitySkill());
        }
        ...
        public override void SetCharacteristics()
        {
            if (SceneManager.GetActiveScene().name != "SandBox")
            {
                ...
                {
                    Data.TypeHeroSoldier.WarriorHero valueWarrior = ProgressService.Progress.PlayerData.TypeHero.WarriorHero;
                    SetAbilityValues(valueWarrior);
                    //SpecialSkillLevelData.LoadStepCurrent(valueWarrior.CurrentStepSpecialAttack);
                }
                else
                {
                    BaseData();
