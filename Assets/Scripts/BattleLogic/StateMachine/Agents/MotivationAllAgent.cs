...
{
    public class MotivationAllAgent : AIAgentBase
    {
        ...
        private void StartAttack(bool specAttack)
        {
            ...
            if (specAttack)
            {
                ...
                var collection = Type == HeroType.Hero ? HeroEnemyList.Heroes : HeroEnemyList.Enemies;
                ...
                foreach (var agent in collection)
                {
                    var motivation = Instantiate(_motivation);
                    motivation?.StartMotivation(agent.GetComponent<AIAgentBase>(), SpecDamage, _motivationTime);
                }
            }
            else
            {
